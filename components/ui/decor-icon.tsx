...
export function DecorIcon({ position, className, ...props }: DecorIconProps) {
  return (
    <svg
      aria-hidden="true"
      className={cn(
        "pointer-events-none absolute z-30 size-6 shrink-0 stroke-[1.5] text-blue-500 transition-all duration-500 opacity-80 group-hover/marquee:opacity-100 group-hover/marquee:scale-110",
        DecorIconVariants({ position, className }),
      )}
      fill="none"
      stroke="currentColor"
      strokeLinecap="round"
